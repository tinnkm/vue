<template lang="html">
  <div class="sales-board">
        ...
        <my-dialog :is-show="isShowPayDialog" @close="hidePayDialog">
          <table class="buy-dialog-table">
            ...
            <tr>
              ...
              <td>{{ period.text }}</td>
              <td>
                <span :key="item.value" v-for="item in versions">{{ item.text }}</span>
              </td>
              <td>{{ price }}</td>
            </tr>
          </table>
        ...
        <check-order :showCheckOrderDialog="isShowCheckOrder" :orderId="orderId" @close="hideCheckOrder"></check-order>
    </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getPrice () {
      let versionString = _.map(this.versions, (ids) => {
        return ids.value
      })
      let reqParams = {
        buyNum: this.buyNum,
        ...
        version: versionString.join(',')
      }

      this.$http.post('/api/getPrice', reqParams).then(
        (res) => {
          this.price = res.data.amount
          // json-serverde post 是往db.json里存值。。。。
          this.price = 121212
        },
        (err) => {
          console.log(err)
